...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function setAuthedUI(authed) {
    ...
    if (!authed) adminShowAlert('Please login to access admin panel.', 'error');
  }
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      const tabBtn = document.getElementById('tabLoginHistory');
      if (tabBtn) tabBtn.click();
    });
  }

  const authed = await adminEnsureAuth();
  await setAuthedUI(authed);

  if (form) {
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      if (!(await adminEnsureAuth())) {
        adminShowAlert('Unauthorized. Please login.', 'error');
        return;
      }
  ...
  async function loadLoginHistory() {
    ...
    try {
      const params = new URLSearchParams({
        select: 'id,timestamp,student_id,students(full_name,course_or_grade)',
      ...
      logs.forEach(log => {
        ...
        tr.innerHTML = `
          ...
          <td>${name}</td>
          <td>${course}</td>
          <td><button class="btn-refresh" data-id="${log.id}">Delete</button></td>
        `;
  ...
  function logsToCsv(logs) {
    ...
    const rows = logs.map(l => {
      ...
      return [l.timestamp, l.student_id, full, course]
  ...
  Object.keys(tabs).forEach(tabId => {
    ...
    if (!btn) return;
    btn.addEventListener('click', () => {
      Object.values(tabs).forEach(sid => {
        const el = document.getElementById(sid);
        if (el) el.style.display = 'none';
      });
      const active = document.getElementById(tabs[tabId]);
